...
namespace BMSRepository
{
    public class UserRepository
    {
        ...
        public List<Book> GetAdminFileApproval()
        {
            return context.Books.Where(b => b.status == 0).ToList();
        }

        public bool GetFileStatusApprove(int bookId)
        {
            Book usr = context.Books.SingleOrDefault(b => b.BookId == bookId);
        ...
        public List<UserFollowingList> GetFollowingList(int id)
        {
            List<UserFollowingList> ls = new List<UserFollowingList>();
            var innerJoin = context.Users.Join(// outer sequence 
                      context.followUsers,  // inner sequence 
                      User => User.userId,    // outerKeySelector
                      followuser => followuser.GuestUserId,  // innerKeySelector
                      (User, followuser) => new UserFollowingList
                      {
                          user_name = User.user_name,
                          MainUserId = followuser.MainUserId,
                          GuestUserId = followuser.GuestUserId
                      }).Where(x => x.MainUserId == id).ToList();
            return innerJoin;
        }
